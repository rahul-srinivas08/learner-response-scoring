...
- **Mean ASR confidence is a real but weak quality signal, checked against outside research, not just assumed.** Consulted arXiv:2503.15124 (Kuhn, Kersken & Zimmermann), which reports ASR confidence correlates with transcription accuracy but is an unreliable error detector on its own — then verified this independently on our own data rather than taking it on faith: mean confidence detects poor responses (score ≤1) at AUC 0.699, actually beaten by plain transcript length (AUC 0.711). That finding — ours, run on this dataset, the paper only motivated the check — is why the feature set uses the *distribution* of word confidences (`conf_min`, `conf_std`, `conf_worst3_mean`, `conf_frac_lt50`) rather than the mean alone; the paper doesn't make that specific recommendation, so it's credited only for the weak-detector finding, not the feature design.
...
**Recommendation: ship Part 2's XGBoost.** XGB+LinReg ensemble scores higher (QWK 0.542, best in the project) but isn't the default — two models to maintain for a 0.01–0.02 gain; documented as the pick if squeezing QWK matters more than simplicity. Plain XGBoost (already tree-based, no ensemble overhead) remains the preferred default otherwise.
...
3. **Trust/abstention layer for Part 3's transformer, unsolved unlike Part 2's** — its one tested uncertainty signal (softmax entropy over the 5-class output) failed outright, AUC 0.46, worse than random (see "Part 3's required engineering topics"), so simply copying Part 2's approach isn't an option here; nothing validated yet for Part 3. Two untested next steps, in priority order: (a) **calibrate first** — temperature-scale the softmax on a held-out set before writing off confidence-based signals entirely, since the likely cause of the failure is an undertrained, miscalibrated model (~1,450 training rows) rather than confidence being fundamentally uninformative; (b) if that doesn't pan out, **borrow Part 2's proven mechanism instead of inventing a new one** — cross-model disagreement between the transformer and XGBoost (the same trick that worked for XGBoost vs. Linear Regression in Part 2's section 6, applied to a different pair of structurally different models). (b) reuses infrastructure already known to work, so it's the safer bet if time is limited, but neither has been run — this is flagged as open, not solved.
4. **A real data-collection strategy, not just synthetic augmentation.** The deeper blocker behind Part 3's data-limited result (item 2 above) is data volume — fine-tuning a transformer only goes so far against ~1,450 training rows; augmentation stretches what's already there but doesn't remove the underlying constraint. Once shipped, use the abstention queue from item 1/3 as a real labeling pipeline: responses routed to human review already need a human judgment, so capturing that judgment as a training label turns production traffic into a genuinely larger, real dataset over time — the actual fix for a data-hungry model, versus augmentation as today's stopgap.

## References

- Kuhn, K., Kersken, V., & Zimmermann, G. (2025). *Evaluating ASR Confidence Scores for Automated Error Detection in User-Assisted Correction Interfaces*. arXiv:2503.15124. https://arxiv.org/abs/2503.15124 — referred to during Part 1's EDA while investigating whether ASR confidence is a reliable quality signal (its finding: correlated with accuracy, but a weak detector on its own). Applied by running the same question directly against this project's own data — an AUC test of mean confidence vs. transcript length for detecting poor responses (0.699 vs. 0.711) — rather than taking the paper's finding on faith; that result is what motivated using confidence-distribution features (`conf_min`, `conf_std`, `conf_worst3_mean`, `conf_frac_lt50`) instead of the mean alone in `feature_engineering.py`.
