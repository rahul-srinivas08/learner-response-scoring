...
**Persistent direction produced more rigor than one test would have:** repeated direction to test the prompt (and language) in the transformer's text input produced four independent variants tested on MiniLM, not one, then a fifth angle — the same tests on a second backbone — that changed the conclusion (see below). The value wasn't the AI catching its own mistake on the first pass; it was direction that kept the question open past the point the AI was ready to call it closed.
