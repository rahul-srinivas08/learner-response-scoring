...
1. **An over-generalized claim, caught by testing further rather than trusting the pattern so far:** after four MiniLM tests all showed adding the prompt hurt, the AI stated this as settled — "not open for revisiting without new training data." Direction to test a second backbone (XLM-R-base) showed the opposite trend on that backbone (QWK rising as context was added, see "Part 3's input design"), proving the original claim was backbone-specific, not a fixed property of the dataset. Corrected in both Part 3's notebook and this report rather than left standing. The most useful of the three, because it's a reasoning error, not an infrastructure one — a reminder that "consistent across N tests" still needs "and I varied the right thing" before it's a general claim.
